...
import { findTreatmentById } from '@/server/features/repositories/treatment-repository';

// ============================================================
// GET /api/treatments/:id — api-spec.md §2.2
// L-1: thin route. findTreatmentById 재사용 (clinics JOIN 포함).
// ============================================================
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!z.string().uuid().safeParse(id).success) {
    return Response.json(
      { error: { code: 'VALIDATION_FAILED', message: 'Invalid treatment id', details: null } },
      { status: 400 },
    );
  }
  ...
  // 5. 단일 조회 (G-2: findTreatmentById 재사용 — clinics JOIN 포함)
  try {
