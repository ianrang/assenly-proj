...
import { findProductById } from '@/server/features/repositories/product-repository';

// ============================================================
// GET /api/products/:id — api-spec.md §2.2
// L-1: thin route. findProductById 재사용 (brand JOIN 포함).
// ============================================================
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!z.string().uuid().safeParse(id).success) {
    return Response.json(
      { error: { code: 'VALIDATION_FAILED', message: 'Invalid product id', details: null } },
      { status: 400 },
    );
  }
  ...
  // 5. 단일 조회 (G-2: findProductById 재사용 — brand JOIN 포함)
  try {
