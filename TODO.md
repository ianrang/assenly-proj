...
| Phase 0    | 37      | 37      | 0       | 0      | ✅      |
| Phase 1    | 62      | 60      | 2       | 0      | ✅      |
| Phase 2    | 138     | 112     | 16      | 10     | 🔶 진행중 |
| Phase 3    | 38      | 15      | 21      | 2      | 🔶 진행중 |
| **MVP 합계** | **287** | **236** | **39**  | **12** |        |
...
| NEW-17a | NEW-17 migration 015/015b Supabase Dashboard 수동 적용 | **대기 (PR 머지 전 필수)**. Supabase Dashboard SQL Editor에서 순차 실행: (1) `015_profile_skin_types_array.sql` — skin_types text[] 컬럼 + 백필 + CHECK + `apply_ai_profile_patch` RPC (2) `015b_apply_ai_journey_patch.sql` — `apply_ai_journey_patch` RPC (lazy-create). 검증: `SELECT proname FROM pg_proc WHERE proname LIKE 'apply_ai_%_patch';` → 2 rows, skin_types 컬럼 + CHECK 제약 존재 확인, 백필 쿼리 `SELECT count(*) FROM user_profiles WHERE skin_type IS NOT NULL AND (skin_types IS NULL OR NOT (skin_types @> ARRAY[skin_type]));` → 0. `016_drop_profile_skin_type.sql`은 프로덕션 코드 배포 + 24~72h 관측 후 적용 | ⬜ |
| NEW-17b | **NEW-17 보안 하드닝 (CRITICAL, PR 머지 전 권장)** | gstack-review 보안 전문가 발견 3건 통합 수정. (1) **p_spec 서버 고정**: `apply_ai_profile_patch` / `apply_ai_journey_patch` 가 p_spec을 인자로 받는 현재 설계는 authenticated 사용자가 PostgREST로 직접 호출 시 aiWritable=true 허용 목록을 위조 가능. spec을 RPC 내부에 하드코딩(또는 DB 메타 테이블)하고 p_spec 인자 제거. (2) **REVOKE authenticated + GRANT service_role**: 서버 경로(chat.ts serviceClient)만 사용하므로 일반 사용자 직접 호출 차단. (3) **DB-side enum CHECK**: skin_types/age_range/budget_level 등의 값을 zod enum과 동등한 CHECK 제약으로 DB 레벨 방어 (bypass 시에도 잘못된 값 저장 불가). MVP 소프트런칭에서는 직접 호출 경로 없어 실익 낮으나, 프로덕션 배포 전 해소 권장 | ⬜ |
...
| NEW-17d | **NEW-17 프로필 편집 UX 경로 설계 (CRITICAL)** | gstack-review 디자인 전문가 발견. 온보딩 완료(`onboarding_completed_at` 설정) 후 사용자가 skin_types/skin_concerns를 수정할 UI 경로가 0개. OnboardingChips는 원샷 게이트로 재노출 안 되고, /profile 의 Edit 버튼은 T3 디프리케이션으로 제거됨. 제품 결정 필요: (A) /profile에 '채팅으로 업데이트' CTA 재도입 + i18n profile.edit 복원 (B) 채팅 내 "프로필 업데이트" tool 도입 (LLM이 변경 요청 감지 → tool 호출 → PUT /api/profile) (C) /profile 전용 편집 폼 추가 (UI 재설계). PRD §4-A 정본 확인 후 설계·구현 | ⬜ |
| NEW-17e | NEW-17 RPC 통합 테스트 (M1 proof on Postgres) | gstack-review 테스트 전문가 발견. `apply_ai_profile_patch` / `apply_ai_journey_patch` Postgres 함수는 프로덕션 AI 쓰기 경로인데 **0 executed test coverage**. merge.ts TS 참조 구현은 22 unit test 통과하나 RPC와 의미 드리프트 가능. integration test 파일(profile-routes, search-filter-routes)은 skin_types 배열로 갱신했으나 migration 015/015b 적용 필요로 실행 안 됨. 작업: (1) NEW-17a 적용 후 integration 테스트 실행 (2) M1 증명 케이스 추가 — skin_types 배열 cap 도달 시 사용자값 불변, age_range scalar 비-null 덮어쓰기 금지 (3) lazy-create journey 케이스 (journey 없는 사용자 AI 추출 시 기본값 'KR'/'seoul' 적용 확인) (4) 선택: T9 RPC↔TS sync test 재도입 | ⬜ |
| NEW-17f | NEW-17 배포 윈도우 안전 (sync trigger) | gstack-review 데이터 마이그레이션 전문가 발견. Migration 015 적용 ~ 코드 배포 완료 사이의 윈도우에서 구 코드가 `skin_type` 단일 컬럼에만 write 시 `skin_types` 미반영으로 데이터 유실. 현재 spec §2.4.1(DO-7)에 운영 런북 있으나 수동 확인 의존. Vercel atomic deploy로 윈도우 분 단위라 MVP 소프트런칭은 허용 가능, 정식 런칭 전 다음 중 선택: (A) migration 015에 BEFORE INSERT/UPDATE trigger 추가(skin_type NOT NULL AND skin_types 미포함 시 ARRAY[skin_type] 합병) + migration 016에서 trigger + 함수 + 구 컬럼 동시 DROP (B) 배포 후 검증 쿼리 자동화 + 탈락 행 발견 시 알람. 정식 런칭 전 (A) 권장 | ⬜ |
...
| NEW-35  | 도메인별 가격 티어 시스템 (방향 A: 티어+중앙값 병기, 하이브리드 우선순위) | 표기: `$$ · ~₩60k` (티어 배지 + 중앙값 축약). 폴백: `$$ · Price varies`. 도메인별 금액 차이 혼동 방지 → ⓘ 아이콘 + tap/hover tooltip ("$$: Mid-range, typically ₩X–₩Y in {domain}"). aria-label에 도메인+절대 범위 명시. 구현: (1) `shared/constants/price-tier.ts` 단일 config — 도메인별 thresholds/medians 응집(S-5/S-8/G-10). **초안 임계값은 NEW-34 실측값 사용**: products `$` <₩23.5k / `$$` ₩23.5k–₩48.5k / `$$$` >₩48.5k, treatments(price_min 기준) `$` <₩50k / `$$` ₩50k–₩200k / `$$$` >₩200k. (2) `beauty/derived.ts`에 `computeTier(domain, price, rangeMin?, rangeMax?)` 순수 함수(VP-1 비개입성) — 우선순위: **실가격 → range 중앙값 → null 폴백**. (3) `shared/utils/format-price-short.ts` — `₩60k` 축약(영어권 통일). (4) `client/ui/PriceTierBadge.tsx` 프리미티브 — 티어+중앙값+ⓘ tooltip + 폴백. (5) ProductCard/TreatmentCard: 기존 `formatPrice`/`formatPriceRange` → `PriceTierBadge` 교체. 테스트 필수: price-tier 단위(도메인×가격 구간×range fallback), format-price-short 단위, PriceTierBadge 컴포넌트(폴백 포함), 카드 렌더 회귀. **의존: NEW-37(스키마) 완료 후 착수**(range 필드 존재 필요) | ⬜   |
| NEW-36  | Phase A — 가격 파이프라인 커버리지 100% 자동 적재 (MVP 승격) | **100% 자동 적재 목표 (관리자 수동 입력 없이)**. 대상: products + treatments. **4단계 fallback 파이프라인**: (1) **36-a 실가격 수집** — 올리브영 Global/브랜드 공식몰 스크래퍼로 `price` 채움 + `price_source='real'` + `price_source_url` 기록. 기존 NEW-27 패턴 확장. G-12 외부 소스 사전 검증(응답·상업적 사용·저작권) 완료 후 진행. (2) **36-b 파이프라인 자동 범위 추정** — 실가격 없는 제품에 카테고리+브랜드+용량(ml/g) heuristic으로 `price_min/max` 채움. NEW-34 실측 quantile 활용. `range_source='estimated-pipeline'` + `price_source_url=수집쿼리`. (3) **36-c AI 범위 보강** — 36-b 실패 시 Gemini 2.5 Flash(temperature=0, JSON strict). 프롬프트: 브랜드/카테고리/용량/제품명. **보수 경계**: `category_p25 ≤ range_min`, `range_max ≤ category_p75 × 1.5`, 위반 시 자동 clip. `range_source='estimated-ai'` + `price_source_url=prompt-version` 기록. (4) **36-d 카테고리 기본값 fallback** — 36-c까지 실패 시 `range_min/max = category p25/p75` 할당. `range_source='category-default'`. **이로써 100% 보증**. **덮어쓰기 우선순위**: manual > real > estimated-pipeline > estimated-ai > category-default (낮은 신뢰도로 높은 것 덮어쓰기 금지). **treatments 동일 규칙 적용** (다만 0% null이라 주로 메타데이터 백필만). 테스트 필수: 4단계 fallback 순수 함수 단위, 우선순위 덮어쓰기 규칙 단위, 보수 clip 로직 단위, mock 소스 통합 테스트. **의존: NEW-37(스키마) 완료 후 착수** | ⬜   |
| NEW-34R | Phase A 검증 — 가격 감사 재실행 + AI 추정 정합성 검증 | NEW-36 완료 후 `npm run audit:price` 재실행 → `docs/audit/price-coverage-{재실행일}.md`. **목표**: 전체 제품 100% 가격 정보(실가격 OR 범위) 보유. 비교: 실가격/범위/AI 추정/카테고리 fallback 각 소스별 채움률. **AI 추정 정합성 검증**: `price_source='estimated-ai'` 샘플 50건 수동으로 실제 시장가 ±50% 이내 여부 확인, 결과를 리포트에 기록. 샘플 선정: 카테고리별 stratified sampling. 실패율 >20% 시 AI 프롬프트 재튜닝 필요 (Phase B 또는 재실행). 100% 미달 또는 카테고리별 편차 심각 시 Phase B 필요 | ⬜   |
