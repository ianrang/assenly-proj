...
| NEW-41  | Explore 페이지 — 도메인별 탐색/검색 화면 | Chat-First 보조 경로. 프로필 기반 개인화 정렬 + 필터 + Load More + 가상 스크롤. PRD: `docs/superpowers/specs/2026-04-19-explore-page-prd.md`. 아키텍처: `docs/superpowers/specs/2026-04-19-explore-page-architecture.md`. 5 에픽: (E1) 라우트+탭+그리드 기반 구조, (E2) 필터 시트+정렬+URL 상태, (E3) /api/explore 서버 scoring+프로필 배너, (E4) @tanstack/react-virtual 가상 스크롤+SWR 캐싱, (E5) 랜딩 Hero CTA 보조 링크. 핵심 설계: Domain Registry 패턴(NFR-8) — 도메인 추가 시 레지스트리 1건+핸들러 1건. Header maxWidth prop 확장. 독자 레이아웃(chat 패턴) | ⬜   |
