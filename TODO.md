...
| NEW-9b  | NEW-9 하드닝: PRD 정합 + 무결성 + 중복 제거 | v1 구현 후 설계 리뷰에서 5개 이슈 발견: (1) PRD §4-A §595 정본 불일치 (concerns 5→7, MAX 2→3). (2) G-2 중복: `OptionGroup` / `ONBOARDING_SKIN_CONCERNS` 상수 / `onboarding.skinType_*` i18n 재사용 안 됨. (3) 재표시 게이트 fragile: `conversationId==null` 대리 지표 → `user_profiles.onboarding_completed_at` 단일 게이트 전환. (4) Skip 상태 영속성 없음 (다음 세션 재표시). (5) journeys 경합 시 중복 active 레코드 가능. 작업: OptionGroup을 `client/ui/primitives/`로 승격, 마이그레이션 014 (`onboarding_completed_at` 컬럼 + `ux_journeys_user_active` 부분 유니크 인덱스 + 기존 중복 dedup), `markOnboardingCompleted` 원샷 서비스(`WHERE IS NULL` + row 존재 precheck, C3), 3단계 handler invariant (profile→journey→mark), Skip API 경로(`{skipped:true}`, create-if-missing으로 기존 데이터 보존), ChatInterface 프로필 병렬 조회 (fail-closed: 500/network→완료로 간주, 404→미완료), i18n `chat.chip*` → `onboarding.skinType_*`/`skinConcern_*` 재사용 + `chat.onboarding.*` 6키만 신규, zod `.strict()` 로 `{skipped,skin_type}` 모순 페이로드 방어(C1). 정본: `docs/03-design/PRD.md` §4-A §595 + `docs/superpowers/specs/2026-04-09-onboarding-and-kit-cta-design.md` §2.1 | ⬜   |
...
| NEW-33  | 채팅 "새 대화" 기능 (reset) | 쿠키/개인화 데이터는 보존, UI만 초기화. 구현: 새 `conversation_id` 발급 방식(기존 row 조작 없음 → 권한 이슈 없음, v0.2 멀티 채팅방과 자연 연결). (1) `POST /api/chat/reset` 라우트 — 새 conversation row 생성 후 id 반환. (2) ChatContent: useChat 메시지 초기화 + `conversationIdRef` 갱신 + 카드 패널 클리어. **프로필 Context는 유지**(개인화 지속). (3) 헤더 좌측 `leftContent` 슬롯에 `MessageSquarePlus` 아이콘 버튼. (4) shadcn `AlertDialog` confirm("대화를 초기화할까요? 이전 대화는 UI에서 사라지지만 데이터는 보존됩니다."). 테스트 필수: reset API 통합 테스트(새 conversation 생성·기존 row 보존), ChatContent reset 동작 단위 테스트 | ⬜   |
