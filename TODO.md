...
|-------|---------|------|--------|------|
| 사전 완료 | 12 | 12 | 100% | ✅ |
| Phase 0 | 37 | 19 | 51% | 🔶 진행 중 |
| Phase 1 | 62 | 0 | 0% | ⬜ 미시작 |
| Phase 2 | 68 | 0 | 0% | ⬜ 미시작 |
| Phase 3 | 36 | 0 | 0% | ⬜ 미시작 |
| **MVP 합계** | **215** | **31** | **14%** | |

**→ 다음: P0-23~P0-28 (인프라 검증) + P0-3 (성공 지표) + P0-11 (버전별 범위)**
...
| P0-11 | 버전별 범위 최종 확정 | P0-4~10 결과 반영. §2 업데이트 | `7.2-ADMIN-REQUIREMENTS.md` §7.2.8 + MASTER-PLAN §2 | ⬜ |
...
| P0-20 | 벡터 검색 정확도 (인메모리) | 5쿼리 5/5 통과. 한국어 포함. pgvector는 §7.5 후 검증 | Top-5 중 3건+ 관련 | 메타데이터 필터 강화 | 🔶 |
| P0-21 | SQL 구조화 검색 성능 | §7.5 Supabase 셋업 후 연기 | < 100ms | 인덱스 추가, 쿼리 최적화 | ⬜ |
| P0-22 | 하이브리드 검색 (인메모리) | 로직 4/4 통과. 필터+벡터 재정렬 정상. DB 통합은 §7.5 후 | 구조화+의미 검색 작동 | 2단계 검색 | 🔶 |
...
| P0-37 | 비용 추정 문서화 | P0-12~36 결과 기반 실측 비용 | MASTER-PLAN §4.2 업데이트 | ⬜ |
