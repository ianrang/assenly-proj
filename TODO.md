...
| P2-60  | Phase A: brands 50+ / ingredients 100+ | **Step 1 코드 보완 완료 (2026-03-31)**. classifier strict 모드(자유 텍스트 분류) + ingredient function spec(strict:false) + FIELD_MAPPINGS(inci_name 매핑) + review-exporter 컬럼 추가. 테스트 6건 추가(총 527). Step 2(brands JSON→enrich→load) + Step 3(ingredients fetch→filter→enrich→load) 대기 중 | M2    | 🔶   |
