...
| ID     | 작업                          | 상세                                                                                                                                                                              | 상태  |
| ------ | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| P2-80  | 프롬프트: 인사 반복 제거              | 매 턴마다 "안녕하세요/Hi!" 인사를 반복하는 문제. prompts.ts §9 "First response" 지시가 매 턴에 적용되는 것으로 LLM이 해석. 첫 턴 전용 조건 분기 또는 "첫 응답에서만 인사" 명시 필요. 히스토리 전달 정상 여부도 검증                                      | ⬜   |
| P2-81  | 프롬프트: 반복 응답 방지              | 같은 말을 계속 반복하는 문제. system-prompt-spec에 반복 방지 지시 없음. "이전 응답과 중복되는 문구를 피하라" + "대화를 진전시켜라" 가이드라인 추가 필요                                                                               | ⬜   |
| P2-82  | 프롬프트: 선제적 추천 유도             | 사용자가 명시적으로 요청할 때까지 추천 안 하는 문제. prompts.ts §9 line 448 "Always answer the user's question first"가 수동적으로 해석됨. 사용자 의도가 K-뷰티 관련이면 적극적으로 tool 호출하여 추천하라는 지시 보강 필요                          | ⬜   |
| P2-83  | 프롬프트: Chat-First 온보딩 강화     | 경로B 진입 시 프로필 수집이 자연스럽게 이루어지지 않음. §9 Tier 1~3 수집 전략이 너무 수동적. 첫 추천 후 자연스럽게 피부타입/고민 1개를 물어보는 패턴으로 강화. PoC는 Gemini 기준 → Claude 모델에 맞춘 재튜닝                                           | ⬜   |
| P2-84  | UI: 추천 카드 컴팩트 레이아웃          | 카드가 세로로 쭈루룩 나오는 문제. 현재 ProductCard h-40 이미지 + 상세 = 카드당 ~250px. 기본 3개 = 750px. 컴팩트 카드(이미지 축소/가로 배치) 또는 카드 요약→상세 확장 패턴 필요. 모바일 퍼스트(L-12) 유지하면서 한 화면에 카드 개요 파악 가능하도록 개선 | ⬜   |
| P2-85  | 버그: 채팅 히스토리 미표시 디버깅         | 페이지 새로고침 시 이전 대화가 보이지 않는 문제. ChatInterface→authFetch→initialMessages→useChat 경로 디버깅. AI SDK 6.x messages prop 마운트 타이밍, card-mapper state 필터링 점검 필요 | ⬜   |
