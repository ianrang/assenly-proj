...
| P2-65a | stores 비소매 데이터 제거                      | other 94건 중 비소매 ~18건(사옥/유통/서비스) 정확한 목록 확정 → DB 삭제 또는 비활성. P2-61b(6건 삭제)와 동일 패턴. Phase B와 병렬 가능                                                            | M3    | ⬜   |
| P2-65b | treatments 분류 수동 보정 (9건)               | critical 4건(Botox Jawline/Chin Filler/Fat Dissolving/Body Lifting — contouring을 uneven_tone으로 force-fit) + moderate 5건(Laser Toning dry 누락, Dermapen dry 누락, Botox Crow's Feet sensitive 누락+dark_circles 오류, Water Glow oily 누락, LED 과다). enriched JSON + DB 직접 수정. Phase B와 병렬 가능 | M3    | ⬜   |
