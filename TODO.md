...
| Phase 0    | 37      | 37      | 0       | 0      | ✅      |
| Phase 1    | 62      | 60      | 2       | 0      | ✅      |
| Phase 2    | 138     | 113     | 16      | 9      | 🔶 진행중 |
| Phase 3    | 38      | 15      | 21      | 2      | 🔶 진행중 |
| **MVP 합계** | **287** | **237** | **39**  | **11** |        |
...
| NEW-17h | NEW-17b code review 후속 (defense-in-depth + 테스트 격리) | NEW-17b PR code review(APPROVE_WITH_FOLLOWUPS) Minor 이슈 2건 후속. (1) **M-1 SET search_path 고정**: migration 017의 4개 함수(`apply_ai_profile_patch`, `apply_ai_journey_patch`, `get_profile_field_spec`, `get_journey_field_spec`)에 `SET search_path = public, pg_temp` 추가 또는 테이블 참조를 `public.user_profiles` / `public.journeys`로 schema-qualify. SECURITY INVOKER + service_role 전용이라 실질 위험 낮으나 defense-in-depth 원칙. migration 017b로 분리 또는 017에 inline ALTER FUNCTION. (2) **M-2 통합 테스트 격리**: `rpc-hardening.integration.test.ts`에서 T3→T2, T6→T4 실행 순서 암묵적 의존 제거. T6 budget_level이 userB T4 생성 journey에 의존, T3이 userA T2 profile에 의존. 각 test 내 `beforeEach`로 자체 seed 또는 전용 user 할당. | ⬜ |
...
| NEW-35  | 도메인별 가격 티어 시스템 (방향 A: 티어+중앙값 병기, 하이브리드 우선순위) | **브레인스토밍 완료 (2026-04-17). 정본: `docs/audit/price-coverage-20260417.md` (N=469, 신뢰도 높음). 04-13 quantile(N=73)은 대체됨.** 표기: `$$ · ~₩60k` (티어 배지 + 중앙값 축약). 폴백: `$$ · Price varies`. 도메인별 금액 차이 혼동 방지 → ⓘ 아이콘 + tap/hover tooltip ("$$: Mid-range, typically ₩X–₩Y in {domain}"). aria-label에 도메인+절대 범위 명시. **확정 임계값 (옵션 B: 라운딩 친화)**: products `$` <₩25k / `$$` ₩25k–₩50k / `$$$` >₩50k (근거: 04-17 p25=25,392 / p75=48,552 → 친화 숫자로 반올림, p75 오차 3%), treatments(price_min 기준) `$` <₩50k / `$$` ₩50k–₩200k / `$$$` >₩200k (04-17 p25=50,000 / p75=200,000과 정확히 일치). **단일 변경점 강제 (S-5/S-8/G-10)**: 모든 임계값·중앙값은 `shared/constants/price-tier.ts` 1파일에서만 정의. 컴포넌트/유틸 어디에도 하드코딩 금지. 값 변경은 이 파일 1곳 수정으로 전역 반영되어야 한다. 구현: (1) `shared/constants/price-tier.ts` — 도메인별 `PRICE_TIER_CONFIG` 단일 export (thresholds·중앙값·라벨). (2) `beauty/derived.ts`에 `computeTier(domain, price, rangeMin?, rangeMax?)` 순수 함수(VP-1 비개입성) — 우선순위: **실가격 → range 중앙값 → null 폴백**. (3) `shared/utils/format-price-short.ts` — `₩60k` 축약(영어권 통일). (4) `client/ui/PriceTierBadge.tsx` 프리미티브 — 티어+중앙값+ⓘ tooltip + 폴백. (5) ProductCard/TreatmentCard: 기존 내부 `formatPrice`/`formatPriceRange` 함수 → `PriceTierBadge` 교체. 테스트 필수: price-tier 단위(도메인×가격 구간×range fallback), format-price-short 단위, PriceTierBadge 컴포넌트(폴백 포함), 카드 렌더 회귀. **선행 작업: NEW-37(스키마) 완료 확인** — migration 013 원격 DB 적용됨(04-17 감사 리포트 생성 성공으로 검증). **착수 조건**: 별도 선행 의존 없음. NEW-36과 독립 병렬 가능 | ⬜   |
| NEW-36  | Phase A — 가격 파이프라인 커버리지 100% 자동 적재 (MVP 승격, 2단계 fallback 확정) | **브레인스토밍 완료 (2026-04-17). 기존 4단계 → 2단계로 스코프 축소 확정 (옵션 B).** 목표: **100% 자동 적재 (관리자 수동 입력 없이)**. 대상: products + treatments. **DB 검증 팩트 (2026-04-17, `scripts/audit/price-coverage.ts` + adhoc 쿼리)**: (i) products 597건 중 `price IS NULL` 128건 (21.4%). 128건 **100% `global.oliveyoung.com` URL 보유**, 전부 2026-04 생성. 원인: NEW-27(URL 보강 시점)에서 URL만 추출, price 추출 누락. (ii) treatments 53건: `price_min/max` 0% null, but `price_source` 53건 전체 NULL (manual seed였으나 source 미기록). (iii) products drift 2건: `price=SET` 이지만 `price_source=NULL`. **2단계 fallback 파이프라인**: (1) **36-a 실가격 보강 패스 (재스크래핑 아님)** — 128건 OY URL 전부 NEW-40의 OY 상세 페이지 파서(`scripts/seed/collect-oy-bestsellers.ts`의 price 추출 함수) 재사용하여 price만 보강. 성공분: `price=<추출값>`, `price_source='real'`, `price_source_url=<기존 URL>`, `price_updated_at=now()`. 같은 도메인·같은 파서 → 실패 위험 낮음. G-12 사전 검증: NEW-27/NEW-40에서 이미 OY Global 상업적 표시 확인 완료. 배치 크기: 128건을 10건씩 13배치 + CRAWL_DELAY_MS=3000 (NEW-40 준수). (2) **36-d 카테고리 기본값 fallback** — 36-a 실패 잔여분에 `price_min=category_p25`, `price_max=category_p75` 할당, `range_source='category-default'`. **이로써 100% 커버 보증**. **스코프 제외 (근거 기록)**: ~~36-b heuristic 범위 추정~~ → 브랜드별 가격대 매핑 테이블(brand_price_tier) 부재, 별도 설계 필요. ~~36-c AI 범위 보강~~ → G-12 상업적 신뢰성 리스크, NEW-34R 조건부 검증 단계였음, UX(PriceTierBadge 폴백 "Price varies" 허용)와 편익 불균형. **treatments 백필 (MVP 1회 batch)**: 53건 `price_source='manual'` 일괄 UPDATE (manual seed임을 명시). products drift 2건 `price_source='real'` 백필. **덮어쓰기 우선순위**: manual > real > category-default (낮은 신뢰도로 높은 것 덮어쓰기 금지). 테스트 필수: 36-a OY 파서 모킹 통합 테스트, 36-d 카테고리 fallback 순수 함수 단위, 우선순위 덮어쓰기 규칙 단위, idempotency(재실행 시 덮어쓰기 방지). **착수 조건**: 별도 선행 의존 없음. NEW-35와 독립 병렬 가능 | ⬜   |
