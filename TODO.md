...
| P2-63  | Phase A: treatments 53건 데이터 수집         | **설계 완료 (2026-04-04)**. Step 1: KB 신규 2건(thread-lift, vitamin-drip). Step 2: 매니페스트 의학적 수정(다운타임 보수적 최대값 + 피부타입/고민 불일치 9건) + 3건 추가(하이드라페이셜/쓰레드리프트/비타민수액) + duration_minutes/session_count 53건. Step 3: YAML→CSV 변환. Step 4: enrich-service FIELD_MAPPINGS+generateSpecs(precautions/aftercare) + review-exporter 6→15컬럼. Step 5: import-csv→enrich→export-review. Step 6: 의학적 정확성 검수. Step 7: DB 적재. 설계서: `docs/superpowers/specs/2026-04-04-p2-63-treatment-data-collection-design.md` | M2    | ⬜   |
...
| P2-63c | lib/ 유틸리티 서브디렉토리 정리                  | Phase A 완료 후. csv-parser, retry, id-generator, db-client → lib/utils/ 이동. 참조 20건+ import 경로 일괄 수정. classifiers/ 정리 완료(P2-61)와 동일 패턴                          | M3    | ⬜   |
| P2-65  | LLM 분류 검증 일괄 (Phase A 완료 후)           | Phase A 전체(stores+clinics+treatments) 분류 정확도 검증. 30~50건 샘플 → 수동 레이블 → LLM 분류 비교 → 정규식 대비 정확도 판단. P2-56r 패턴 적용. StoreTypeClassifier 인터페이스 활용           | M3    | ⬜   |
