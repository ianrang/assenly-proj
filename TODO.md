...
| NEW-41  | Explore 페이지 — 도메인별 탐색/검색 화면 | **진행중 (2026-04-19)**. Chat-First 보조 경로. PRD: `docs/superpowers/specs/2026-04-19-explore-page-prd.md`. 아키텍처: `docs/superpowers/specs/2026-04-19-explore-page-architecture.md`. **완료**: (E1) 라우트+탭+그리드+API, (E2) 필터 시트+정렬+URL 상태, (E3) 프로필 배너+서버 scoring, (E5) 랜딩 Hero 보조 링크. Domain Registry 패턴(NFR-8). 단위 테스트 1078건, 통합 테스트 20건, tsc/lint/build 통과. **잔여**: (1) 가상 스크롤 재적용(@tanstack/react-virtual, headless 환경 이슈로 일반 그리드 임시 사용 중) (2) 브라우저 QA 시각적 검증 (3) PR 생성. 브랜치: `feat/NEW-41-explore-page`, 8 커밋 | 🔶 진행중 |
