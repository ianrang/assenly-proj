...
| **U-6** | 브랜드 이미지 사용 허가 | ~~높~~ **확정** | — | **검증 완료 (2026-03-23)** | **P2-V3 완료.** 5개 브랜드 공개 프레스킷 포털 미확인(PR 문의 필요). 업계 표준: 리뷰/추천 사이트의 제조사 마케팅 이미지 사용은 fair use 범위. 화해/글로우픽 동일 관행. **전략: 브랜드 공식 사이트 제품 이미지 사용 + "Image: [Brand]" 출처 표시 + 변형 금지. 확보 불가 시 placeholder** |
...
```
즉시 확정:    U-8 (네이버 이미지 사용 안 함), U-5 (네이버 쇼핑 API MVP 사용 보류 → 수동 폴백)
완료:         U-5 (2026-03-23 검증 완료)
