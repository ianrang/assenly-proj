...
### Stage 1 (MVP v0.1): 수동 큐레이션 + API 보조 + CSV 일괄

| 항목 | 방법 | 도구 |
|------|------|------|
| **products 200+** | 운영팀 수동 큐레이션. 올리브영/시코르 웹사이트 참조 → 구글시트/CSV 작성 → 일괄 업로드. 네이버 쇼핑 API를 관리자 검색 보조 도구로 활용 | CSV 로더 + 관리자 앱 + AI 분류/번역 |
...
| **purchase_links** | 올리브영 글로벌 어필리에이트 가입 → 트래킹 링크 생성 | Involve Asia 대시보드 |
| **목표** | 200제품, 50매장, 30클리닉 — 개인화 정확도 우선 | |

### Stage 2 (v0.2): 브랜드/업체 셀프 등록 — 화해 모델

| 항목 | 방법 | 도구 |
|------|------|------|
| **products 확장** | 브랜드사가 제품 등록 양식(엑셀) 작성 → 제출 → 운영팀 검수 → DB 적재. 화해와 동일 검증된 모델 | 제휴업체 앱 (MASTER-PLAN §2.2) |
| **clinics/salons** | 클리닉/살롱이 직접 등록 (강남언니 모델) | 제휴업체 앱 |
| **어필리에이트 피드** | CJ 네트워크 등 어필리에이트 제품 피드 API 연동 가능성 검토 → 제품 데이터 자동 수신 | CJ Product Feed API |
| **목표** | 제품 500+, DOM-3/DOM-4 추가 | |

### Stage 3 (v0.3+): 어필리에이트 피드 자동화 + 사용자 기여

| 항목 | 방법 |
|------|------|
| **제품 피드 자동화** | CJ/Involve Asia 어필리에이트 피드 API 연동 → 제품명·가격·이미지·링크 자동 동기화 |
| **사용자 기여 (UGC)** | 사용자가 제품 리뷰 작성 시 신규 제품 등록 제안 → 운영팀 검수 |
| **가격 자동 갱신** | 어필리에이트 피드에서 실시간 가격 업데이트 수신 |
| **목표** | 제품 1000+, 자동 갱신, 수동 의존도 최소화 |

### 어필리에이트 프로그램 = 제품 데이터의 합법적 소스

에센리의 어필리에이트 가입 → 판매자(올리브영 등)가 **계약에 의해** 제품 데이터(이름, 가격, 이미지, 설명, 구매링크)를 제공한다. 이것은 크롤링이나 API 약관 우회가 아닌 **정상적인 비즈니스 계약**이다.

| 어필리에이트 | 제공 데이터 | 이미지 사용 | 참조 |
|------------|-----------|-----------|------|
| 올리브영 글로벌 | 배너, 트래킹 링크, 리포팅 | 프로모션 용도 허용 (계약) | [Involve Asia](https://involve.asia/blog/olive-young-affiliate-program/) |
| CJ 네트워크 | **제품 피드 API** (제품명, 가격, 이미지URL, 설명, 카테고리) | 어필리에이트 약관에 의해 허용 | [CJ Product Feed](https://developers.cj.com/docs/data-imports/product-feeds) |

> **MVP 액션**: 올리브영 글로벌 어필리에이트 프로그램 가입 (가입 조건: 소셜미디어 계정 보유 회원). 가입 후 배너/링크 도구 확보 → purchase_links에 트래킹 코드 포함.

### 네이버 쇼핑 API — P2-V1 판정 수정

네이버 쇼핑 API는 **관리자 보조 도구로 활용 가능**하다. "전면 보류"에서 "DB 자동 적재만 보류, 검색 보조 허용"으로 수정:

| 사용 방식 | 허용 |
|----------|------|
| 관리자가 API로 제품 검색 → 가격/카테고리 확인 → CSV에 수동 기입 | **허용** |
| 사용자에게 네이버 쇼핑 검색 결과를 실시간 표시 | **허용** (API 본래 목적) |
| API 결과를 자동으로 자체 DB에 대량 적재 | **약관 확인 후** (MVP에서는 사용 안 함) |
...
**MVP 판정: S2 사용 보류.** 수동+CSV 폴백 채택. 추후 네이버 개발자센터 로그인 → 약관 정독 후 재평가 가능.
...
| **U-5** | **네이버 쇼핑 API 이용 범위** | **판정 수정 (2026-03-24)** | — | **재평가 완료** | **P2-V1 재평가.** 초기 판정 "전면 보류"는 과도. 네이버 쇼핑 API는 공개 API이며 검색 도구로서의 사용은 문제 없음. **수정 판정: (1) 관리자 검색 보조 도구로 활용 허용 (제품명/가격/카테고리 참조 → CSV에 수동 기입) (2) DB 자동 대량 적재만 보류 (약관 원문 확인 후 재평가) (3) 어필리에이트 프로그램 가입 시 제품 피드를 계약에 의해 수신하는 것이 정식 경로** (§1.2 Stage 2~3 참조) |
...
```
즉시 확정:    U-8 (네이버 이미지 사용 안 함), U-5 (S2 MVP 사용 보류)
사전 검증 완료(명세/샘플 수준): U-2 (식약처 명세 확인), U-3 (CosIng 샘플 확인), U-6 (이미지 잠정 전략)
M1 보완 필수:  U-1 (AI 분류 PoC), U-2 (식약처 실제 호출), U-3 (CSV 직접 검증+function 확인), U-6 (브랜드 PR 문의)
M2 시점:      U-4 (중복 제거), U-7 (가격 현실성)
```
...
|--------|---------|------|--------|------|
| products (수동 + API 보조) | ~15분 | 200 | ~50시간 | 네이버 쇼핑 API를 관리자 검색 보조로 활용 (가격/카테고리 참조). CSV 작성 → AI 분류 → 전수 검수. 어필리에이트 피드 확보 시 공수 대폭 감소 |
...
> 네이버 쇼핑 API를 관리자 검색 보조로 활용하여 products 공수 유지 (~50시간). 코딩(~5-7주)과 데이터 입력(~3주)을 병행. M1→M2→M3 단계적 진행. v0.2에서 어필리에이트 피드 + 브랜드 셀프 등록 도입 시 수동 의존도 대폭 감소.
