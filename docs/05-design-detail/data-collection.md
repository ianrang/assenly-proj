...
| 항목 | 방법 | 도구 |
|------|------|------|
| **products 200+** | 운영팀 수동 큐레이션. **브라우저에서** 올리브영/시코르/화해 웹사이트 참조 → 구글시트/CSV 작성 → 일괄 업로드 + AI 분류/번역. ~~네이버 쇼핑 API~~ 사용 불가 (약관 7.3③) | CSV 로더 + 관리자 앱 + AI 분류/번역 |
...
```
즉시 확정:    U-8 (네이버 이미지 사용 안 함), U-5 (MVP API 호출 안 함, 브라우저 검색만)
사전 검증 완료(명세/샘플 수준): U-2 (식약처 명세 확인), U-3 (CosIng 샘플 확인), U-6 (이미지 잠정 전략)
M1 즉시 착수: U-9 (어필리에이트 가입 신청 — 승인 리드타임 확보)
M1 보완 필수: U-1 (AI 분류 PoC), U-2 (식약처 실제 호출), U-3 (CSV 직접 검증), U-6 (브랜드 PR 문의), U-10 (CJ-올리브영 관계), U-11 (어필리에이트 약관 제품정보 범위)
...
|--------|---------|------|--------|------|
| products (수동 + AI 보강) | ~15분 | 200 | ~50시간 | 브라우저에서 올리브영/시코르 웹사이트 수동 참조 → 구글시트/CSV 작성 → AI 분류/번역 → 전수 검수. 어필리에이트 피드 확보(Stage 2+) 시 공수 감소 |
