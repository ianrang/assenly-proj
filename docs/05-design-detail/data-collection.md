...
```
확정 완료:    U-2 (식약처 실제 호출 검증), U-3 (CosIng CSV 직접 검증), U-5 (네이버 약관 확정), U-6 (이미지 — placeholder 확정, D-14), U-8 (네이버 이미지 안 함), U-12 (쿠팡 보류), U-13 (올리브영 글로벌 약관)
M1 보완 필수: U-1 (AI 분류 PoC), U-9 (올리브영 어필리에이트 가입)
