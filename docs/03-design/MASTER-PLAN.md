...
| ID | 작업 | 상세 | 산출물 | 상태 |
|----|------|------|--------|------|
| P0-1 | PRD v2.0 → v2.2 리뷰 + 정리 | v2.1: 비즈니스 이슈 20건 + 기술 혼입 19개소 제거, 사용자 결정 14건. v2.2: 중복 제거, §3.3↔§4-A 역할 분리, 부록 간소화 (953→876줄) | PRD v2.2 | ✅ |
...
| P0-11 | 버전별 범위 최종 확정 | P0-4~10 결과 반영. §2 업데이트 | `7.2-ADMIN-REQUIREMENTS.md` §7.2.8 + 본 문서 §2 | ⬜ |
...
| P0-17 | 점진적 개인화 추출 | Tool 방식 (extract_user_profile). 6개 변수 테스트: UP-1 100%, JC-1 83%, JC-3 100%, JC-4 100%, UP-4 100%, BH-4 100%. 전체 93%. U-4 결정: (a) Tool 방식 채택 | 80%+ 정확 추출 | 별도 추출 tool 정의 | ✅ |
| P0-18 | 멀티 모델 전환 검증 | Gemini 2.0-flash→2.5-flash 전환 성공 (동일 코드). Anthropic provider 초기화 성공. Config 3-provider 분기 동작. Claude/OpenAI 런타임은 키 확보 후 검증 | 코드 변경 없이 모델 전환 성공, tool_use 호환성 확인 | 어댑터 패턴 자체 구현 | ✅ |
...
| U-2 | 관리자 인증 방식 | ~~(a) Supabase Auth + RLS (b) 자체 인증~~ → **(c) Google Workspace SSO + JWT 확정** | ✅ P0-6 |
| U-3 | 벡터 인덱스 MVP 포함 | (a) MVP에 생성 (b) v0.2로 연기 | Phase 0 (P0-19) |
| U-4 | 개인화 추출 방식 | ~~(a) 별도 tool (b) 후처리 (c) LLM 지시~~ → **(a) Tool 방식 확정** (P0-17: 93% 정확도) | ✅ P0-17 |
...
| X-4 | PRD §4-A BH-2 vs DB beauty_history | PRD에 "재구매 의사" 필드가 있으나 DB에 해당 컬럼 없음 (A-7 연계) |
| X-5 | PRD §4-B 공통 vs DB-SCHEMA | PRD: "상태 관리(활성/비활성/임시 휴업)" 3값 → DB: status TEXT DEFAULT 'active' (허용값 미정의). PRD에 허용값 정의 완료 |
| X-6 | PRD §4-B DOM-1 vs DB-SCHEMA products | brands에 FK 제약 없음 (products.brand_id UUID — REFERENCES brands(id) 누락) |
...
**→ 다음 작업: P0-19~P0-22 (RAG 기술 검증) + P0-3 (성공 지표) + P0-11 (버전별 범위) + P0-1 사용자 결정 12건**
