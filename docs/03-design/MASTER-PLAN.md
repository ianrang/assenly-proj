...
| ID | 작업 | 상세 | 산출물 | 상태 |
|----|------|------|--------|------|
| P0-1 | PRD v2.0 리뷰 | 비즈니스 이슈 20건 + 기술 혼입 19개소 제거 + DB/TDD 이관 8건. 해소 8건, 사용자 결정 12건, P0-2 이관 8건 | PRD 리뷰 리포트 (`docs/04-poc/prd-review.md`) | ✅ |
| P0-2 | PRD ↔ DB-SCHEMA 정합성 해소 | §14 불일치 6건(X-1~X-6) + P0-1 이관 8건 해소 | 스키마/TDD 수정 | ⬜ |
...
| P0-6 | 관리자 인증 요구사항 | 로그인 방식, 세션 관리, 비밀번호 정책 | `7.2-ADMIN-REQUIREMENTS.md` §7.2.3 | ⬜ |
| P0-7 | 다국어 데이터 입력 UX | 6개 언어 데이터 입력 UI 방식 결정 | `7.2-ADMIN-REQUIREMENTS.md` §7.2.4 | ⬜ |
| P0-8 | 이미지 업로드 요구사항 | 허용 형식, 크기 제한, 저장 방식 | `7.2-ADMIN-REQUIREMENTS.md` §7.2.5 | ⬜ |
| P0-9 | 데이터 검증 규칙 | 필수 필드, 값 범위, 관계 무결성 | `7.2-ADMIN-REQUIREMENTS.md` §7.2.6 | ⬜ |
| P0-10 | 감사 로그 요구사항 | 누가 언제 무엇을 변경했는지 추적 | `7.2-ADMIN-REQUIREMENTS.md` §7.2.7 | ⬜ |
...
**→ 다음 작업: Phase 0-2 (P0-2: PRD ↔ DB-SCHEMA 정합성 해소) + P0-1 사용자 결정 필요 항목 16건 논의**
